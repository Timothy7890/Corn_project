...
def main(input_txt):
    ...
    colored_data = cluster_and_color(pcd_data)
    save_colored_data(colored_data, input_txt.split(".")[0])

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Cluster point cloud data')
    parser.add_argument('--input13', type=str, required=True, help='Path to the input 13-column point cloud txt file')
    parser.add_argument('--eps', type=float, default=0.4, help='DBSCAN clustering radius (default: 0.4)')
    parser.add_argument('--threshold', type=float, default=0.8, help='Threshold for semantic filtering (default: 0.8)')
